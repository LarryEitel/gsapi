# -*- coding: utf-8 -*-
from gsapi.models.contact import CntX
...
class TestCnt(TestCase):
    ...
    def test_get(self):
        ...
        for i in range(len(sort_test['values'])):
            print docs_found[i][sort_test['fld']]
            print sort_values[i]
...
class TestCntX(TestCase):
    ...
    def test_post_one(self):
        print "## TestCntX.test_post_one"
        print '''### INSERT NEW CntX:'''
        ...
        sample = {
            ...
            "lNam":"dayal",
            "lvOn":{"$date": 1347893866298},
            "oBy":{"$oid":"50468de92558713d84b03fd1"},
            "rBy":{"$oid":"50468de92558713d84b03fd8"},
            "gen":'m',
            "emails" : [{
                "email" : "john@doe.com"
            }],
            "cnt_id":{"$oid":"50468de92558713d84b03fd2"},
            "role_id":{"$oid":"50468de92558713d84b03fda"},
            "weight":1.0
        }
        data = json.dumps(sample)
        route = "/" + self.collection
    ...
    def test_get(self):
        ...
        resp       = self.load_sample('contactxs')
        assert resp['status'] == 200
        ...
        print "\nVerify GET all docs:"
        query               = "/" + self.collection
        test_expected_count = 2 # Note the filter on _cls
        print "RAW REQUEST:\n%s\n" % (route + query)
        ...
        # hard coded, would rather convert from sample_doc
        # test_datetime       = sample_doc[test_field]
        test_value       = 1347644492471

        where_test          = '{"%s":{"$date":%d}}' % (test_field, test_value)
        test_expected_count = 2
        ...
        # hard coded, would rather convert from sample_doc
        # test_datetime       = sample_doc[test_field]
        test_value       = "50468de92558713d84b03fd7"
        ...
        # Expected Values:
        # the following identifies correct results given sample data. If sample data is refreshed, make sure you identify what field to sort and expected return values
        sort_test = {'fld':'fNam', 'values':['Betty','Marry']}
        sort='[{"fNam":"1"}]'
        query = '/%(collection)s?sort=%(sort)s' % {'collection':self.collection, 'sort':sort}
        print "\nVerify SORT:"
        print "RAW REQUEST:\n%s\n" % (route + query)
        ...
        sort_values = sort_test['values']
        for i in range(len(sort_test['values'])):
            print docs_found[i][sort_test['fld']], sort_values[i]
            assert docs_found[i][sort_test['fld']] == sort_values[i]
        print 'Success'
